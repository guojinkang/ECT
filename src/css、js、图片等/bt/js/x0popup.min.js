...
x0popup=x0p=function(){
	function t(){
		var t="";
		return t+="width: "+x.width+";",t+="height: "+x.height+";",
	...
	function n(){
		...
		break;
		case"custom":t+='<i class="xi" style="background: url(\''+x.iconURL+"') no-repeat center center; background-size: 100% 100%;\"></i>"}
		return t+="</div>"}
	function e(){
		var t="";
		if(
				c=x.buttons,
				null==c&&(c=[],1!=x.showCancelButton&&(0==x.showCancelButton||"warning"!=x.type&&"input"!=x.type)||c.push({type:"cancel"}),
						"text"==x.type||"input"==x.type?c.push({type:"info"}):c.push({type:x.type})),0==c.length)
			return"";
		var n=c.length,e="width: "+(100/n).toFixed(2)+"%; width: calc(100% / "+n+");";t+='<div id="x0p-buttons" class="buttons">';
	...
	function l(){i("x0popup"),i("x0p-overlay")}
	function a(){
	...
	function o(){
		for(var t=0;t<c.length;++t){
			...
			!function(t,e){
				n.addEventListener("click",function(){u(t,e)})}(c[t].type,c[t].showLoading)}}
	function u(t,n){var e=document.getElementById("x0popup");
	if(null!=e){var o=document.getElementById("x0p-input");
	if("cancel"!=t&&null!=g&&null!=o&&null!=x.inputValidator){
		var u=x.inputValidator(t,o.value);if(null!=u){i("x0p-input-error");
		var r=document.getElementById("x0p-text-anchor");
		return void r.insertAdjacentHTML("beforeend",'<div id="x0p-input-error" class="error">'+u+"</div>")}}clearTimeout(m),1==n?a():l(),
				null!=v&&v(t,null==o?null:o.value)}}
	
	function r(t){
		if(t.hasOwnProperty("text"))
			return t.text;
		var c=document.getElementById("a").value;
		var b="<a href='deletePb?publishname="+c+"'>确认</a>";
		switch(t.type){
	...
	function s(){var t=x.inputColor;return null==t?"":"<style>#x0p-input:focus { border-color:"+t+"; color:"+t+"; }</style>"}
	function p(t){var n=document.createElement("div");
	...
	else{for(var f in d)x[f]=d[f];void 0!=arguments[1]&&(v=arguments[1])}
	var y="",h=null==x.icon&&("text"==x.type||"input"==x.type),
	g=null!=x.inputType?x.inputType:"input"==x.type?"text":null,b=null==x.inputValue?"":x.inputValue,w=null==x.inputPlaceholder?"":x.inputPlaceholder,C=e();x.overlay&&(y+='<div id="x0p-overlay" class="x0p-overlay'+(x.animation&&x.overlayAnimation?"":" no-animation")+'"></div>'),y+='<div id="x0popup" class="x0p '+x.theme+(0==x.animation?" no-animation":"")+(0==c.length?" no-button":"")+'" style="'+t()+'">',y+='<div class="content">',y+=h?'<div class="text-pure-wrapper">':n()+'<div class="text-wrapper">',y+='<div id="x0p-text-anchor" class="text-anchor">',y+='<div class="title">'+x.title+"</div>",
			null!=x.text&&(y+='<div class="text">'+(x.html?x.text:p(x.text))+"</div>"),
			null!=g&&(y+='<div class="input">'+s()+'<input id="x0p-input" type="'+g+'" placeholder="'+w+'" value="'+b+'"></div>'),y+="</div>",y+="</div>",y+="</div>",y+=C,y+="</div>",l();
	var T=document.getElementsByTagName("body");T[0].insertAdjacentHTML("beforeend",y);
	var k=document.getElementById("x0p-input");
	null!=g&&k.focus(),o(),
	null!=x.autoClose&&(m=setTimeout(function(){u("timeout")},x.autoClose))},x0popup.setDefault=x0p.setDefault=function(){var t=arguments[0];
